...
function ConfigSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="space-y-3 rounded-lg border border-fd-border bg-fd-muted/30 p-4">
...
export function LayoutShowcase() {
  ...
  const updateParams  = useCallback((updates: Partial<LayoutParams>)  => setParams((p)  => ({ ...p,  ...updates })), []);
  const updateMonitor = useCallback((updates: Partial<MonitorParams>) => setMonitor((m) => ({ ...m, ...updates })), []);

  const applyMonitorPreset = useCallback((preset: typeof MONITOR_PRESET_OPTIONS[number]) => {
    setMonitor({
      width:     preset.width,
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <ConfigSection title="Monitor">
          <div className="flex flex-wrap gap-2">
            {MONITOR_PRESET_OPTIONS.map((preset) => (
              <button
                key={preset.label}
                onClick={() => applyMonitorPreset(preset)}
          ...
          <div className="flex items-center gap-2 text-xs text-fd-muted-foreground">
            ...
            <input
              ...
              className="w-20 rounded border border-fd-border bg-fd-background px-2 py-1 text-fd-foreground"
            />
            <span>×</span>
            <input
          ...
          <div className="pt-1 text-xs text-fd-muted-foreground">
            {logicalWidth} × {logicalHeight} @ {monitor.scale}x
            {monitor.isPortrait ? " portrait" : " landscape"} ({effectiveWidth} × {effectiveHeight} phys)
          </div>
        </ConfigSection>
        ...
        {layoutInfo.hasScroller && (
          <ConfigSection title="Scroller">
            ...
            <RangeSlider label="Proportion" value={params.scrollerDefaultProportion}   min={0.3} max={1.0} step={0.1} onChange={(v) => updateParams({ scrollerDefaultProportion: v })} formatValue={pct} />
            <RangeSlider label="Single"     value={params.scrollerDefaultProportionSingle} min={0.3} max={1.0} step={0.1} onChange={(v) => updateParams({ scrollerDefaultProportionSingle: v })} formatValue={pct} />
      ...
      <div className="flex justify-center">
        <div
          className="relative overflow-hidden rounded-2xl border-2 border-fd-border bg-gradient-to-br from-fd-muted/30 to-fd-muted/10 shadow-2xl"
          style={{ width: previewWidth, height: previewHeight }}
        >
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-lg font-medium text-fd-muted-foreground/30">
          ...
          {rects.map((rect, index) => (
            <WindowRect
              key={index}
              rect={rect}
              focused={index === params.focusedWindow}
